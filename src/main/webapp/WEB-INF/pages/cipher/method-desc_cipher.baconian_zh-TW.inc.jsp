...
<p>原始培根密碼使用 24 個字母的字母表，其中 I/J 和 U/V 分別視為同一個字母。後來的第二版為 A 到 Z 的每個字母都分配了獨立編碼。DenCode 支援這兩種字母表。</p>

<h4>原始的 24 字母表</h4>
<p>在原始字母表中，I 和 J 共用一個編碼，U 和 V 也共用一個編碼。編碼時，J 會按 I 處理，V 會按 U 處理。</p>
...
<h4>第二版：26 字母表</h4>
<p>在第二版中，從 A 到 Z 的每個字母都有獨立的培根編碼，包括 I、J、U 和 V。</p>
...
<p>作為隱寫術使用時，先把秘密訊息轉成 A/B 序列，再準備一段足夠長的表面文字來承載這個序列。表面文字中的每個字母用兩種字體之一書寫：一種讀作 A，另一種讀作 B。文字看起來仍然是普通句子，但按五個一組讀取字體差異，就能還原隱藏訊息。培根也描述過用於手寫的雙字母表，讓大寫和小寫字母各有兩種替代形態。</p>
