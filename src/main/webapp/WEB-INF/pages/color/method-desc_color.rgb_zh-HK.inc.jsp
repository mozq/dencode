...
%><h3>關於 RGB 顏色</h3>
<p>RGB 顏色是透過組合紅、綠、藍三種光的成分來表示顏色的顏色模型。它廣泛用於顯示器、網頁、圖像資料等螢幕上的顏色表示。</p>
...
<div class="table-responsive">
	<table class="table">
		<thead>
			<tr><th>顏色</th><th>RGB 表示</th><th>十六進制表示</th></tr>
		</thead>
		<tbody>
...
<p>RGB 顏色有函數形式和十六進制形式。例如，紅色可以表示為 <code>rgb(255 0 0)</code>、<code>rgb(100% 0% 0%)</code>、<code>#ff0000</code>、<code>#f00</code>。</p>

<p>如要包含透明度，請加入 Alpha 值。例如，半透明紅色可以表示為 <code>rgb(255 0 0 / 50%)</code> 或 <code>#ff000080</code>。</p>

<p>DenCode 的 RGB 顏色轉換支援以下表示選項。</p>

<div class="table-responsive">
	<table class="table">
		<caption>RGB 顏色（十六進制）</caption>
		<thead>
...
<p><code>0xAARRGGBB</code> 不是 CSS 的標準十六進制顏色表示，但它是有時用於程式或應用程式設定的 ARGB 格式。開頭的 <code>AA</code> 表示 Alpha，後續的 <code>RR</code>、<code>GG</code>、<code>BB</code> 表示紅、綠、藍成分。</p>
...
<div class="table-responsive">
	<table class="table">
		...
		<tbody>
			<tr><td>sRGB</td><td><code>rgb(...)</code><br><code>color(srgb ...)</code></td><td><code>rgb(255 0 0)</code><br><code>color(srgb 1 0 0)</code></td><td>Web 和 CSS 中常見的 RGB 色彩空間。十六進制顏色代碼和普通 <code>rgb()</code> 基本作為 sRGB 處理。<code>color(srgb ...)</code> 是 CSS Color Module Level 4 的格式。</td></tr>
			...
			<tr><td>Display P3</td><td><code>color(display-p3 ...)</code></td><td><code>color(display-p3 1 0 0)</code></td><td>比 sRGB 色域更廣的色彩空間，可能用於廣色域顯示器和流動裝置。</td></tr>
...
<p>使用廣色域 RGB 色彩空間指定的顏色，可能無法在 sRGB 螢幕或普通十六進制顏色代碼中準確表示。DenCode 轉換為 <code>#RRGGBB</code> 或數值 <code>rgb(R G B)</code> 時，超出 sRGB 可表示範圍的成分會按適合 <code>0</code> 到 <code>255</code> 範圍的方式處理。另一方面，選擇 <code>color(display-p3 ...)</code> 或 <code>color(a98-rgb ...)</code> 等色彩空間時，可以顯示為所選色彩空間的成分值。</p>
